  ...
  ws("$JOB_NAME") { // This must be the name of the role otherwise ansible won't find the role
    try {
      wrap([$class: 'AnsiColorBuildWrapper', colorMapName: 'xterm']) {
        stage('Checkout') {
          deleteDir()
          git url: "https://github.com/hmcts/ansible-management.git", branch: "new-winjenkins-role", credentialsId: "jenkins-public-github-api-token"
...
EOF
          ...
          cp -a ./roles/bootstrap-role/templates/resolv.conf.j2 ./roles/bootstrap-role/tasks/templates/resolv.conf.j2
            ...
            if [ "$HOSTNAME_PARAM" == "" ]; then
              ...
              ansible-config view   
              python inventory/azure_rm.py | python -m json.tool
              ansible-playbook -vvv -e ansible_host=172.16.4.27 -i  inventory/azure_rm.py --tags "$ANSIBLE_TAGS" run_bootstrap.yml 
            else
